...
function execute(body) {
    return new Promise(function(resolve, reject){
        var res = https.request(body.options, function(res){
        ...
        switch(body.options.headers['content-type']) { // request
            ...
            case AEMM.mimetypes.symlink:
                res.end(JSON.stringify(body.schema));
                break;
            ...
            case AEMM.mimetypes.article:
                var readStream = fs.ReadStream(body.upload.file);
                if(readStream) {
                ...
                break;
            default:
                reject('HttpObject: Non-supported content-type ' + body.options.headers['content-type']);
        }
    });
}
    ...
    return new Promise(function(resolve, reject){
        ...
        switch (res.headers['content-type']) {
            ...
            case AEMM.mimetypes.gif:
                var matches = AEMM.matchContentUrlPath(request.path); // 1 pubId, 3, entityType , 4, entityName 5. subpath 6. filename
            ...
            case AEMM.mimetypes.octetstream:
                switch(request.method) {
                    ...
                    case 'DELETE':
                        ...
                        break;
                    default:
                        reject('HttpObject: Method not supported', request.method);
                }
                break;
