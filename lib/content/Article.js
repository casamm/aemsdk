    ...
    return iterateArticleDirectory(body.article.src)
        .then(function(files){ // create manifest.xml
            if(body.article.generateManifest == undefined || body.article.generateManifest == true) {
                ...
                writeStream.write('  <manifest version="3.0.0" targetViewer="33.0.0" dateModified="' + new Date().toISOString() + '">\n');
...
Article.prototype.uploadArticle = function(body) {
    body.options = {
        hostname: AEM.endPoints.ingestion,
        path: '/publication/' + body.schema.publicationId + '/article/' + body.schema.entityName + ';version=' + body.schema.version + '/contents/' + (body.article.type || 'folio'),
        method: 'PUT',
        headers: {
            'content-type': AEM.mimetypes.article,
            'accept': AEM.mimetypes.json
        }
    };
...
function parseFile(file) {
    return new Promise(function(resolve, reject){
        Promise.all([md5(file), stat(file)]).then(function(result){
            resolve({md5: result[0], type: AEM.mimetypes[path.extname(file).substr(1)], href: file, length: result[1].size});
        });
    });
}
...
function iteratedir(dir) { // list of files in directory and subdirectories (recursive)
    return readdir(dir)
        .then(function(list){
