...
Authentication.prototype.requestToken = function(body) {
    return new Promise(function(resolve, reject){
        var options = {
            hostname: AEM.endPoints.authentication,
            path: '/ims/token/v1?grant_type=device&scope=AdobeID,openid' +
            '&client_id=' + AEM.credentials.clientId +
            '&client_secret=' + AEM.credentials.clientSecret +
            '&device_id=' + AEM.credentials.deviceId +
            '&device_token=' + AEM.credentials.deviceToken,
            method: 'POST',
            headers: {
